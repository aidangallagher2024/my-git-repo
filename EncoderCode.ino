...
const long interval = 100;       // Interval for 1 second (1000 milliseconds)
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(encoderPinA), countImpulse, RISING);

   // Initialize serial communication for debugging
  Serial.begin(115200);   //match jetson baud rate
  //Serial.begin(500000);   //match jetson baud rate
  }
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    Serial.print(turns*10);
    Serial.print(" | in per second: ");
    Serial.print(turns*.75*3.14*10);
    Serial.print(" | ft per second: ");
    Serial.println(turns*.75*3.14/12*10);*/
    Turn2 = turns*.75*3.14/12*10;
